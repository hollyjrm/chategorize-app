...
app.post(
  ...
  catchAsync(async (req, res) => {
    ...
    const newChat = new Room(
      { name: req.body.chatname, owner: person._id },
      { autoIndex: false }
    );
...
app.get(
  ...
  catchAsync(async (req, res) => {
    ...
    res.render(__dirname + "/views/chatname.ejs", {
      currUser: req.user,
      users,
      room,
    });
...
io.on("connection", (socket) => {
  ...
  socket.on("joinRoom", async function ({ chatname, id }) {
    ...
    Message.find({ room: wantedRoomId })
      .populate("sender")
      .then((result) => {
    ...
    socket.on("chat message", async function (msg) {
      ...
      Room.findById(wantedRoomId)
        .populate({ path: "users", select: "email -_id" })
        .then((result) => {
          // all people in the room
  ...
  socket.on("disconnect", function () {
    let connectionMessage =
      socket.username + " Disconnected from Socket " + socket.id;
